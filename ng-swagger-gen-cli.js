...
function ngSwaggerGen(options) {
  ...
  if (!options.compare && !options.generate && !options.update) {
  ...
  $RefParser
    ...
    .then(
      ...
      (err) => {
        console.error(`Error reading swagger location ${options.swagger}: ${err}`);
      },
    )
...
function doGenerate(swagger, options) {
  ...
  if (swagger.swagger !== '2.0') {
    console.error('Invalid swagger specification. Must be a 2.0. Currently ' + swagger.swagger);
    process.exit(1);
  }
  ...
  var generate = function(template, model, file) {
    var code = Mustache.render(template, model, templates).replace(/[^\S\r\n]+$/gm, '');
  ...
  for (var modelName in models) {
    ...
    generate(templates.model, model, path.join(modelsOutput, model.modelFile + '.ts'));
    if (options.generateExamples && model.modelExample) {
      ...
      generate(templates.example, model, path.join(modelsOutput, model.modelExampleFile + '.ts'));
    }
  }
    ...
    modelFiles.forEach((file, index) => {
      ...
      for (var modelName in models) {
        ...
        if (
          ...
          (basename == model.modelExampleFile + '.ts' && model.modelExampleStr != null)
        ) {
          ok = true;
          break;
        }
  ...
  generate(templates.strictHttpResponse, {}, path.join(output, 'strict-http-response.ts'));
  ...
  for (var serviceName in services) {
    ...
    generate(templates.service, service, path.join(servicesOutput, service.serviceFile + '.ts'));
  }
  ...
  {
    generate(templates.baseService, applyGlobals({}), path.join(output, 'base-service.ts'));
  }
}
...
function applyTagFilter(models, services, options) {
  ...
  for (var serviceName in services) {
    var include = (!included || included.indexOf(serviceName) >= 0) && (!excluded || excluded.indexOf(serviceName) < 0);
    if (!include) {
      // This service is skipped - remove it
      console.info('Ignoring service ' + serviceName + ' because it was not included');
  ...
  if (ignoreUnusedModels) {
    ...
    for (var modelName in models) {
      ...
      if (!allDependencies.has(model.modelClass)) {
        ...
        console.info('Ignoring model ' + modelName + ' because it was not used by any service');
...
function collectDependencies(dependencies, model, models) {
  ...
  if (model.modelDependencies) {
    model.modelDependencies.forEach((dep) => collectDependencies(dependencies, dep, models));
  }
}
  ...
  for (name in swagger.definitions) {
    ...
    if (descriptor.properties != null) {
      ...
      if (descriptor.modelProperties.length > 0) {
        descriptor.modelProperties[descriptor.modelProperties.length - 1].propertyIsLast = true;
      }
    }
  ...
  switch (property.type) {
    ...
    case 'object':
      ...
      if (property.additionalProperties) {
        if (memberCount++) def += ', ';
        type = typeof property.additionalProperties === 'object' ? propertyType(property.additionalProperties) : 'any';
        allTypes.push(type);
        def += '[key: string]: ' + type;
      }
      def += '}';
...
function operationId(given, method, url, allKnown) {
  ...
  if (generate) {
    console.warn("Operation '" + method + "' on '" + url + "' defines no operationId. Assuming '" + id + "'.");
  } else if (duplicated) {
    console.warn(
      "Operation '" +
        method +
...
function processServices(swagger, models, options) {
  ...
  for (var url in swagger.paths) {
    ...
    for (var method in path || {}) {
      ...
      var id = operationId(def.operationId, method, url, descriptor.operationIds);
      ...
      for (var p = 0; p < parameters.length; p++) {
        ...
        var paramDescriptor = {
          ...
          paramToJson: param.in === 'formData' && paramTypeNoNull !== 'Blob' && paramTypeNoNull !== 'string',
      ...
      var operation = {
        ...
        operationPath: url,
        operationPathExpression: toPathExpression(operationParameters, paramsClass, url),
      ...
      operation.operationIsOther = !['void', 'number', 'boolean'].includes(actualType);
      ...
      operation.operationIsPrimitiveArray =
        !modelResult && (resultType.toString().includes('Array<') || resultType.toString().includes('[]'));
  ...
  for (name in services) {
    ...
    for (i = 0; i < service.serviceOperations.length; i++) {
      ...
      for (var code in op.operationResponses) {
        ...
        var actualDeps = status < 200 || status >= 300 ? errorDependencies : dependencies;
